...
class WrappedVim:
    ...
    def host_eval(self, code):
        return self._vim.call('DenshiInternalEval', code)
...
def test_commands(vim):
    """The :Denshi command is registered and doesn't error"""
    vim.command('Denshi')


@pytest.mark.xfail
def test_no_python_file(vim):
    """If no Python file is open, Denshi doesn't handle the current file"""
    assert vim.host_eval('plugin._cur_handler is None')


def test_python_file(vim, tmp_path):
    """If a Python file is open, Denshi handles the current file"""
...
def test_switch_handler(vim, tmp_path):
    ...
    assert not vim.eval('get(b:, "denshi_attached", v:false)')
    assert vim.host_eval('plugin._cur_handler is None')
    assert vim.host_eval('len(plugin._handlers)') == 2
    vim.command('bprev')
    assert vim.eval('get(b:, "denshi_attached", v:false)')
...
def test_wipe_buffer(vim, tmp_path):
    ...
    # Without calling a Denshi command first, the subsequent assertions
...
def test_option_filetypes(start_vim):
    ...
    vim = start_vim(['--cmd', 'let g:denshi#filetypes = []'], file='foo.py')
    assert vim.host_eval('plugin._cur_handler is None')

    vim = start_vim(['--cmd', 'let g:denshi#filetypes = ["html", "php"]'], file='foo.py')
    assert vim.host_eval('plugin._cur_handler is None')

    vim = start_vim(['--cmd', 'let g:denshi#filetypes = ["html", "php"]'], file='foo.php')
    assert vim.host_eval('plugin._cur_handler is not None')


def test_option_excluded_hl_groups(start_vim):
    vim = start_vim(['--cmd', 'let g:denshi#excluded_hl_groups = ["global", "imported"]'], file='')
    ...
    # available.
    assert vim.host_eval('plugin._cur_handler._parser._excluded == ["denshiGlobal", "denshiImported"]')


def test_option_mark_selected_nodes(start_vim):
    vim = start_vim(['--cmd', 'let g:denshi#mark_selected_nodes = 0'], file='')
    ...
    vim = start_vim(['--cmd', 'let g:denshi#mark_selected_nodes = 2'], file='')
    vim.current.buffer[:] = ['aaa', 'aaa', 'aaa']
    vim.wait_for_update_thread()
...
def test_option_no_default_builtin_highlight(start_vim):
    ...
    vim = start_vim(['--cmd', 'let g:denshi#no_default_builtin_highlight = 0'], file='')
    vim.current.buffer[:] = ['len']
...
def test_option_always_update_all_highlights(start_vim):
    ...
    vim = start_vim(['--cmd', 'let g:denshi#always_update_all_highlights = 1'], file='')
    vim.current.buffer[:] = ['aaa', 'aaa']
    old = get_ids()
    vim.current.buffer[:] = ['aaa', 'aab']
    new = get_ids()
...
def test_syntax_error_sign(start_vim):
    ...
    vim = start_vim(['--cmd', 'let g:denshi#error_sign_delay = 0'], file='')
    vim.current.buffer[:] = ['+']
    vim.wait_for_update_thread()
    time.sleep(SLEEP)
    ...
    vim = start_vim(['--cmd', 'let g:denshi#error_sign = 0'], file='')
    vim.current.buffer[:] = ['+']
    vim.wait_for_update_thread()
    time.sleep(SLEEP)
    with pytest.raises(neovim.api.nvim.NvimError):
    ...
    vim = start_vim(['--cmd', 'let g:denshi#error_sign_delay = 1.0'], file='')
    vim.current.buffer[:] = ['+']
    vim.wait_for_update_thread()
    time.sleep(SLEEP)
    with pytest.raises(neovim.api.nvim.NvimError):
...
def test_option_tolerate_syntax_errors(start_vim):
    ...
    vim = start_vim(['--cmd', 'let g:denshi#tolerate_syntax_errors = 0'], file='')
    vim.current.buffer[:] = ['a+']
    time.sleep(SLEEP)
    num_nodes = vim.host_eval('len(plugin._cur_handler._parser._nodes)')
    assert num_nodes == 0
...
def test_option_update_delay_factor(start_vim):
    vim = start_vim(['--cmd', 'let g:denshi#update_delay_factor = 2'], file='')
    time.sleep(SLEEP)
...
def test_option_self_to_attribute(start_vim):
    ...
    vim = start_vim(['--cmd', 'let g:denshi#self_to_attribute = 0'], file='')
    vim.current.buffer[:] = buf
    vim.current.window.cursor = [2, 16]
...
def test_goto(start_vim):
    ...
    time.sleep(SLEEP)
    vim.command('Denshi goto function next')
    wait_for(lambda: vim.current.window.cursor == [2, 1])
    vim.command('Denshi goto class prev')
    wait_for(lambda: vim.current.window.cursor == [1, 0])
    vim.command('Denshi goto class last')
    wait_for(lambda: vim.current.window.cursor == [4, 0])
    vim.command('Denshi goto class first')
    wait_for(lambda: vim.current.window.cursor == [1, 0])
...
def test_goto_name(start_vim):
    ...
    vim.current.buffer[:] = ['aaa, aaa, aaa']
    time.sleep(SLEEP)
    vim.command('Denshi goto name next')
    ...
    wait_for(lambda: vim.current.window.cursor == [1, 0])
    time.sleep(SLEEP)
    vim.command('Denshi goto name prev')
    wait_for(lambda: vim.current.window.cursor == [1, 10])
    time.sleep(SLEEP)
    vim.command('Denshi goto name prev')
    wait_for(lambda: vim.current.window.cursor == [1, 5])
...
def test_goto_hl_group(start_vim):
    ...
    time.sleep(SLEEP)
    vim.command('Denshi goto parameterUnused first')
...
def test_enable_disable(start_vim):
    ...
    vim.command('Denshi disable')
    wait_for(no_handler)
    vim.command('Denshi enable')
    wait_for(num_nodes(1))
    vim.command('Denshi disable')
    wait_for(no_handler)
    vim.command('Denshi enable')
...
def test_enable_by_filetype(start_vim):
    ...
    assert vim.eval('b:denshi_attached')
    assert vim.host_eval('plugin._cur_handler is not None')
    assert vim.host_eval('len(plugin._handlers)') == 1
    vim.command('set ft=php')
    assert not vim.eval('b:denshi_attached')
    assert vim.host_eval('plugin._cur_handler is None')
    assert vim.host_eval('len(plugin._handlers)') == 0
